...
if st.sidebar.button("Prediksi"):
    ...
    if trs_type in ['SALES', 'RETUR']:
        ...
        def rank_products(data, trs_type_label, is_retur=False):
            # Pastikan kolom yang dibutuhkan ada
            required_columns = ['TRS_TYPE', 'NAMABARANG', 'QTYSALES']
            if not all(col in data.columns for col in required_columns):
                return pd.DataFrame(columns=['Rank', 'NAMABARANG', 'Total Produk', 'Rata-rata Produk'])

            # Encode trs_type_label menggunakan LabelEncoder
            trs_type_encoded = le_trs_type.transform([trs_type_label])[0]

            # Filter data berdasarkan jenis transaksi yang di-encode
            filtered_data = data[data['TRS_TYPE'] == trs_type_encoded]

            # Cek apakah hasil filter kosong
            if filtered_data.empty:
                st.write(f"Data kosong untuk tipe transaksi: {trs_type_label}")
                return pd.DataFrame(columns=['Rank', 'NAMABARANG', 'Total Produk', 'Rata-rata Produk'])

            # Group by produk dan hitung total serta rata-rata
            ...
            # Sortir berdasarkan 'Total Produk'
            if is_retur:
                # Untuk retur, urutkan secara ascending karena kita ingin nilai negatif terbesar di atas
                ranking = ranking.sort_values(by='Total Produk', ascending=True).reset_index(drop=True)
            else:
                # Untuk sales, urutkan secara descending
                ranking = ranking.sort_values(by='Total Produk', ascending=False).reset_index(drop=True)

            # Tambahkan kolom Rank
            ranking['Rank'] = ranking.index + 1

            # Ubah kode produk kembali ke nama aslinya
            ranking['Nama Barang'] = le_namabarang.inverse_transform(ranking['NAMABARANG'])

            # Susun ulang kolom agar Rank berada di posisi pertama
            ranking = ranking[['Rank', 'Nama Barang', 'Total Produk', 'Rata-rata Produk']]

            return ranking

        # Tampilkan data untuk debugging
        # st.write("Data yang diterima:", data)

        # Tampilkan ranking produk berdasarkan sales
        st.subheader('Perankingan Produk Sales')
        ranking_sales = rank_products(data, 'SALES')
        st.write(ranking_sales)

        # Tampilkan ranking produk berdasarkan retur, dengan sorting ascending untuk nilai retur (negatif terbanyak di atas)
        st.subheader('Perankingan Produk Retur')
        ranking_retur = rank_products(data, 'RETUR', is_retur=True)
